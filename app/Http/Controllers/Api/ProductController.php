...
class ProductController extends Controller
{
    ...
    {
        $a = Producto::select('id','nombre','descripcion','precio_inicial','img1','path1','precio_actual'
        ,'estado')->get();
    ...
    public function store(Request $r)
    {
        ...
        if($r->img1 <> null){
            $producto->img1 = $r->file('img1')->store('productos/'.$producto->nombre,'public');
            $url1                = Storage::url($producto->img1);
            $producto->path1     = $url1;
        }
        if($r->img2 <> null){
            $producto->img2 = $r->file('img2')->store('productos/'.$producto->nombre,'public');
            $url2                = Storage::url($producto->img2);
            $producto->path2     = $url2;
        }
        if($r->img3 <> null){
            $producto->img3 = $r->file('img3')->store('productos/'.$producto->nombre,'public');
            $url3                = Storage::url($producto->img3);
            $producto->path3     = $url3;
        }
        if($r->img4 <> null){
            $producto->img4 = $r->file('img4')->store('productos/'.$producto->nombre,'public');
            $url4                = Storage::url($producto->img4);
            $producto->path4     = $url4;
        }
        if($r->img5 <> null){
            $producto->img5 = $r->file('img5')->store('productos/'.$producto->nombre,'public');
            $url5                = Storage::url($producto->img5);
            $producto->path5     = $url5;
        }
        if($r->img6 <> null){
            $producto->img6 = $r->file('img6')->store('productos/'.$producto->nombre,'public');
            $url6                = Storage::url($producto->img6);
            $producto->path6     = $url6;
        }
        if($r->img7 <> null){
            $producto->img7 = $r->file('img7')->store('productos/'.$producto->nombre,'public');
            $url7                = Storage::url($producto->img7);
            $producto->path7     = $url7;
        }
        if($r->img8 <> null){
            $producto->img8 = $r->file('img8')->store('productos/'.$producto->nombre,'public');
        ...
        if($r->img9 <> null){
            $producto->img9 = $r->file('img9')->store('productos/'.$producto->nombre,'public');
